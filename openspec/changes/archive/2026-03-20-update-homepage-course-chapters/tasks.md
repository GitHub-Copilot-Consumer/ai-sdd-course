...
- [x] 1.3 git commit: `test: add homepage course links validation script`
...
- [x] 2.3 git commit: `fix: update homepage course chapter links to match current lesson slugs`

## 3. 驗證

- [x] 3.1 執行測試腳本 `scripts/test-homepage-links.sh`，確認所有 9 個連結驗證通過（綠燈）
- [x] 3.2 在本地以 `hugo server` 啟動站點，目視確認首頁顯示正確的 9 個章節連結且各連結可正常跳轉
