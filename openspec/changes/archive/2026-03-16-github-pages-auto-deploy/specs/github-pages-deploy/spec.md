...
- **建置步驟順序**：
  ...
  3. `peaceiris/actions-hugo@v2`（`hugo-version: '0.120.4'`，`extended: true`）
...
- **THEN** 檔案 MUST 存在，且 MUST 包含 `go-version: '1.21'`、`hugo-version: '0.120.4'`、`hugo -s site/ --minify`、`-b ${{ env.GITHUB_PAGES_URL }}`、`publish_dir: ./site/public` 等明確字串
